...
class DrpConformancePathError(ValueError):
...
def load_drp_conformance_bundle(path: str | Path) -> dict[str, Any]:
    ...
    if not bundle_raw.strip():
        raise DrpConformancePathError(
            "bundle path must not be empty or whitespace-only",
            condition="drp_conformance_bundle_empty",
        )
...
def write_drp_conformance_report(path: str | Path, report: Mapping[str, Any]) -> None:
    ...
    if not output_raw.strip():
        raise DrpConformancePathError(
            ...
            condition="drp_conformance_output_empty",
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except DrpConformancePathError as exc:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "drp_conformance_path_invalid",
