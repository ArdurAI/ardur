...
def test_ask_user_question_post_tool_use_records_hash_only_user_choice_context(tmp_path, monkeypatch):
    token = _issue_wildcard_test_passport(tmp_path)
    monkeypatch.setenv("ARDUR_MISSION_PASSPORT", token)
    monkeypatch.setenv("VIBAP_HOME", str(tmp_path))
    monkeypatch.setenv("ARDUR_CC_HOOK_DIR", str(tmp_path / "chain"))

    from vibap._hashing import canonical_json, sha256_hex
    from vibap.claude_code_hook import handle_pre_tool_use, handle_post_tool_use
    ...
    tool_input = {
        "questions": [
            {
                "id": "q1",
                "question": "Which deployment should I use?",
            }
        ]
    }
    tool_response = {
        "response": "use the private staging cluster",
        "answers": {"q1": "staging"},
        "annotations": [{"kind": "note", "text": "human private note"}],
    }
    ...
    post_claims = pyjwt.decode(lines[1].strip(), options={"verify_signature": False})
    claude_measurements = post_claims["measurements"]["claude_code"]
    context = claude_measurements["user_choice_context"]
    assert context["schema_version"] == "ardur.claude_code.user_choice_context.v0.1"
    assert context["source"] == "claude_code_post_tool_use.tool_response"
    assert context["mode"] == "mixed"
    assert context["question_count"] == 1
    assert context["answer_count"] == 1
    assert context["annotation_count"] == 1
    assert context["response_present"] is True
    assert context["response_sha256"] == {
        "alg": "sha-256",
        "value": sha256_hex("use the private staging cluster"),
    }
    assert context["answers_hash"] == {
        "alg": "sha-256",
        "value": sha256_hex(canonical_json(tool_response["answers"])),
    }
    assert context["annotations_hash"] == {
        "alg": "sha-256",
        "value": sha256_hex(canonical_json(tool_response["annotations"])),
    }

    canonical_tool_response = json.dumps(
        tool_response,
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    )
    assert post_claims["result_hash"] == {
        "alg": "sha-256",
        "value": sha256_hex(canonical_tool_response),
    }

    signed_payload = json.dumps(post_claims, sort_keys=True)
    assert "use the private staging cluster" not in signed_payload
    assert "staging" not in signed_payload
    assert "human private note" not in signed_payload
