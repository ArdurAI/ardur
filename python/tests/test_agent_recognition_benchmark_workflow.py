...
BUDGET = (
    ...
    / "kernelcapture"
    / "testdata"
    / "agent-recognition-benchmark-budget-v0.4.json"
)
...
def test_automatic_ci_requires_v4_budget_and_manual_profiles_are_explicit() -> None:
    ...
    assert (
        "manual CI dispatch is collecting budget-independent v0.4 evidence" in workflow
    )
    assert (
        "manual release profile is a budget-independent experiment and never substitutes for required CI"
        in workflow
    )


def test_main_promotion_uses_reviewed_daemon_capable_reference() -> None:
    workflow = WORKFLOW.read_text(encoding="utf-8")

    assert "pull_request:\n    paths:" in workflow
    assert (
        "branches: [dev]"
        not in workflow.split("pull_request:", 1)[1].split("workflow_dispatch:", 1)[0]
    )
    assert (
        "PROMOTION_BOOTSTRAP_REFERENCE_SHA: 7a2167f543671bba4fc20a8d3702f5ae6d6315df"
    ) in workflow
    assert '[ "$PR_BASE_REF" = "main" ] && [ "$PR_HEAD_REF" = "dev" ]' in workflow
    assert '! git cat-file -e "$reference_sha:go/cmd/ardur-kernelcaptured"' in workflow
    assert 'reference_sha="$PROMOTION_BOOTSTRAP_REFERENCE_SHA"' in workflow
    assert '[ "$reference_sha" = "$SOURCE_SHA" ]' in workflow
    assert 'reference_sha="$(git rev-parse "$SOURCE_SHA^")"' in workflow
    assert 'git merge-base --is-ancestor "$reference_sha" "$SOURCE_SHA"' in workflow
    assert 'git cat-file -e "$reference_sha:go/cmd/ardur-kernelcaptured"' in workflow
...
def test_workflow_builds_and_records_an_exact_same_vm_reference() -> None:
    ...
    assert (
        'if [ -n "$(git status --porcelain --untracked-files=all)" ]; then' in workflow
    )
    assert (
...
def test_published_recognition_workflow_matches_authoritative_workflow() -> None:
    assert WORKFLOW_MIRROR.read_text(encoding="utf-8") == WORKFLOW.read_text(
        encoding="utf-8"
    )
