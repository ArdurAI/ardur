...
def test_gemini_fixture_cli_rejects_file_project_dir_without_partial_writes(tmp_path):
    repo_root = Path(__file__).resolve().parents[2]
    caller_home = tmp_path / "caller-home"
    ardur_home = tmp_path / "ardur-home"
    ...
    assert output["error"] == "gemini_cli_fixture_project_dir_not_directory"
    assert output["condition"] == "gemini_cli_fixture_project_dir_not_directory"
