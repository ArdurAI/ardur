"""Focused regression tests for gemini-cli-fixture path validation.
...
def _run_fixture(*args: str, env: dict[str, str], repo_root: Path) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-m", "vibap.cli", "gemini-cli-fixture", *args],
        text=True,
        capture_output=True,
        check=False,
        env=env,
...
def test_gemini_fixture_rejects_home_existing_file(tmp_path: Path) -> None:
...
def test_gemini_fixture_rejects_chain_dir_existing_file(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    caller_home = tmp_path / "caller-home"
    ardur_home = tmp_path / "ardur-home"
...
def test_gemini_fixture_rejects_keys_dir_existing_file(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    caller_home = tmp_path / "caller-home"
    ardur_home = tmp_path / "ardur-home"
    fixture_home = tmp_path / "fixture-home"
...
def test_gemini_fixture_rejects_project_dir_dangling_symlink(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    caller_home = tmp_path / "caller-home"
    ardur_home = tmp_path / "ardur-home"
    fixture_home = tmp_path / "fixture-home"
...
def test_gemini_fixture_rejects_home_dangling_symlink(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    caller_home = tmp_path / "caller-home"
    ardur_home = tmp_path / "ardur-home"
    project_dir = tmp_path / "project"
...
def test_gemini_fixture_valid_inputs_still_work(tmp_path: Path) -> None:
...
def test_gemini_fixture_rejects_project_dir_empty(tmp_path: Path) -> None:
    ...
    output_text = json.dumps(output, sort_keys=True)
    assert output["ok"] is False
    assert output["error"] == "gemini_cli_fixture_project_dir_empty"
    assert output["condition"] == "gemini_cli_fixture_project_dir_empty"
    assert "empty" in output["message"].lower()
    assert "Traceback" not in output_text
    assert str(tmp_path) not in output_text
    # No CWD pollution: no fixture artifacts created.
...
def test_gemini_fixture_rejects_project_dir_omitted(tmp_path: Path) -> None:
