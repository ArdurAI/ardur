...
def test_bundle_empty_string_is_structured(
    ...
    assert captured.out == ""
    assert report["ok"] is False
    assert report["error"] == "drp_conformance_path_invalid"
    assert report["condition"] == "drp_conformance_bundle_empty"
...
def test_output_empty_string_is_structured(
    ...
    assert report["ok"] is False
    assert report["error"] == "drp_conformance_path_invalid"
    assert report["condition"] == "drp_conformance_output_empty"
    assert "Traceback" not in captured.err
...
def test_bundle_empty_raises_specialized_error() -> None:
    with pytest.raises(DrpConformancePathError) as exc_info:
        load_drp_conformance_bundle("")
    assert exc_info.value.condition == "drp_conformance_bundle_empty"


def test_output_empty_raises_specialized_error() -> None:
    with pytest.raises(DrpConformancePathError) as exc_info:
        write_drp_conformance_report("   ", {"ok": True})
    assert exc_info.value.condition == "drp_conformance_output_empty"
