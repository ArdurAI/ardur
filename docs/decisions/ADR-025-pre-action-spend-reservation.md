...
11. When a tool is governed by both typed risk and spend budgets, risk preflight
    reserves first and spend reserves second. Any non-`PERMIT` result or
    evaluation exception enters one idempotent compensation path that releases
    every reservation actually acquired. The resulting event can honestly carry
    both the spend `budget_delta` and typed risk remaining-budget evidence.
