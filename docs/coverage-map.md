...
| `ardur run -- <cli>` host-observer lifecycle | **Root-process lifecycle only** — root PID, command, `run_command` (actual argv when adapter wrapping differs), `cwd` (absolute working directory), `duration_budget_s` (caller-set time budget), started-at, wall-clock duration, exit code, exit signal. Zero-privilege, no kernel daemon, works on macOS/Linux with any CLI. `capture_tier=host-observer`. | `process_lifecycle` object in governance result: `root_pid`, `command`, `run_command` (when differing), `cwd` (when captured), `duration_budget_s` (when set), `started_at`, `wall_clock_s`, `exit_code`, `exit_signal`, `capture_tier` |
