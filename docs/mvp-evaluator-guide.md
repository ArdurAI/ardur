...
[`scripts/verify-mvp.sh`](../scripts/verify-mvp.sh).
...
[`scripts/run-no-key-mvp-demo.py`](../scripts/run-no-key-mvp-demo.py), which
...
- **Capture boundary:** Ardur governs at the tool-call boundary. See
  [`docs/coverage-map.md`](coverage-map.md) for current coverage and roadmap
  boundaries.
...
- [No-Key MVP Demo](guides/no-key-mvp-demo.md)
