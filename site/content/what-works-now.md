...
| CLI | Protocol and Personal commands including `issue`, multi-mode `verify`, `anchor`, `receiver-attestation-fixture`, `attest`, `start`, `hub`, `setup`, `status`, `doctor`, `doctor-claude-code`, `run`, `profile init`, `protect claude-code`, `claude-code-hook`, and `claude-code-report` | {{< repo-link "docs/reference/cli.md" "CLI reference" >}} |
...
| Protocol docs | Mission Declaration, Delegation Grant, Execution Receipt, EAT profile, Transparency Anchor, Receiver Attestation, Verifier Contract, conformance profiles, IDM extension, revocation | {{< repo-link "docs/specs/README.md" "Specs index" >}} |
...
keyed evidence for configured paths, but do not prove universal observation or
