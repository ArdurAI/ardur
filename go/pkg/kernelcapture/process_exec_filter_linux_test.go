...
func TestProcessExecRecognitionBasenameKeyUsesExactNULTerminatedShape(t *testing.T) {
	key, err := processExecRecognitionBasenameKey("codex")
	if err != nil {
		t.Fatal(err)
	}
	if got := string(key[:5]); got != "codex" {
...
func TestProcessExecRecognitionBasenameKeyRejectsPathAndOverflow(t *testing.T) {
	for _, value := range []string{"/usr/bin/codex", strings.Repeat("x", maxAgentRecognitionExecutableBasenameBytes+1)} {
		if _, err := processExecRecognitionBasenameKey(value); err == nil {
			t.Fatalf("invalid executable basename %q unexpectedly accepted", value)
		}
	}
}
